...
PROXY_PORT = 52309
# Bind to 0.0.0.0 so the HTTP server is reachable from the LAN — required for
# remote OAuth authentication on headless devices (phone visits the /auth/start
# URL on the Kodi device's LAN IP).  Audio streaming URLs still use 127.0.0.1
# (PROXY_URL_HOST) so Kodi connects locally without IPv6 dual-stack delays.
PROXY_HOST = "0.0.0.0"
# Host used in URLs that Kodi fetches locally (audio streams).  127.0.0.1 avoids
# the IPv6 dual-stack resolution delay that "localhost" can cause on some devices.
PROXY_URL_HOST = "127.0.0.1"
