...
class _SpotifyOSDPlayerMonitor(xbmc.Player):
    ...
    def _clear(self) -> None:
        global _liked_state_track_id
        # Guard: only clear if Kodi is truly not playing.  During track transitions
        # onPlayBackEnded for the OLD track can fire *after* __on_track_started for
        # the NEW track has already set properties, wiping the liked state.
        if self.isPlaying():
            return
    ...
    def __on_track_started(self, track_id: str, duration_sec: float) -> None:
        ...
        def _set_liked_state():
            try:
                ...
                # Discard result if a newer track started while the API call was in flight.
                if _liked_state_track_id != track_id:
                    log_msg(
                        f"Liked-state response for {track_id} arrived but current track "
                        f"is now {_liked_state_track_id!r}, discarding.",
                        LOGDEBUG,
                    )
                    return
        ...
        try:
            ...
            if prebuffer_enabled:
                ...
                def _deferred_prebuffer():
                    ...
                    # 5s provides room for error on slow connections.
                    time.sleep(5.0)
