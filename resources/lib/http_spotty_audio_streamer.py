...
from utils import ADDON_ID, ADDON_WINDOW_ID, LOGDEBUG, get_cached_auth_token, get_spotipy_client, log_msg
...
class HTTPSpottyAudioStreamer:
    def __init__(
        ...
        # Pre-created thread pool for fire-and-forget track-started callbacks.
        # On ARM (CoreELEC), threading.Thread.start() costs ~76 ms — enough to push
        # the HTTP response past Kodi's QueueNextFileEx timeout (~160 ms), causing
        # "Open - Unhandled exception" and a cascade-skip of the entire playlist.
        # submit() to a pre-warmed pool takes ~microseconds instead.
        self.__event_pool = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="spotify-evt"
        )
    ...
    def stop(self) -> None:
        ...
        self.__event_pool.shutdown(wait=False, cancel_futures=True)
    ...
    def spotty_stream_audio_track(
        ...
        if is_new_track:
            ...
            # Use the pre-warmed thread pool instead of threading.Thread.start()
            # to avoid the ~76 ms thread-creation cost on this ARM device.
            self.__event_pool.submit(self.__on_track_started, track_id, float(duration))
    ...
    def _handle_wav_request(
        ...
        elif request_range == "bytes=0-":
            # Return 206 with Content-Range so CurlFile/CFileCache learns the exact
            # file size, preventing Kodi from allocating cachemembuffersize (1GB) per
            # stream. On this 32-bit ARM Kodi process (~3GB virtual address space),
            # two simultaneous 1GB CFileCache allocations exhaust virtual memory,
            # causing every QueueNextFileEx to fail with "Open - Unhandled exception".
            # With Content-Range, CFileCache allocates min(file_size+1MB, 1GB) ≈ 40MB.
            status = "206 Partial Content"
            content_range = f"bytes 0-{range_end - 1}/{file_size}"
            log_msg(
                f"bytes=0- full-range as 206, content length = {range_end - range_begin}.", LOGDEBUG
            )
        else:
            status = "206 Partial Content"
            try:
    ...
    def toggle_track_like(self, track_id: str) -> bottle.Response:
        ...
        try:
            ...
            self.__event_pool.submit(clear_flag)
