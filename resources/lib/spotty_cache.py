...
class SpottyDownloader:
    ...
    # Delays between spotty retry attempts after a session conflict (0 PCM bytes, rc=0).
    # Longer delays give Spotify's backend more time to release the previous librespot
    # session.  With [1, 3, 5] the last attempt fired at ~9 s — often before the backend
    # finished cleanup — so all 4 attempts failed and the cascade started.
    # [2, 5, 10] keeps the total under 30 s (Kodi's HTTP timeout) while giving the
    # final attempt at ~17 s when most session releases are complete.
